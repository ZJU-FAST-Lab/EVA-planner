...
public:
	...
	double safety_dist_;
	bool LowMpc(Eigen::Vector2d& start_pt, Eigen::Vector2d& end_pt);
	bool HighMpcc(const Eigen::Vector3d& start_state, bool if_adaptive);
    ...
    std::vector<Eigen::Vector2d> global_path_ , local_path_;
	std::vector<std::vector<Eigen::Vector3d>> high_mpcc_traj_;
	std::vector<Eigen::Vector3d> high_mpcc_traj_pos_, high_mpcc_traj_vel_, high_mpcc_traj_ref_;
