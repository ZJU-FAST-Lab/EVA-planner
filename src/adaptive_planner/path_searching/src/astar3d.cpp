...
namespace adaptive_planner {
    ...
    void Astar3d::setParam(ros::NodeHandle &nh) {
    ...
    void Astar3d::setEnvironment(const SDFMap::Ptr &env) {
        this->sdf_map = env;
    }

    Vector3d Astar3d::gridIndex2coord(const Vector3i &index) {
        Vector3d pt;
    ...
    Vector3i Astar3d::coord2gridIndex(const Vector3d &pt) {
        Vector3i idx;
    ...
    Eigen::Vector3d Astar3d::coordRounding(const Eigen::Vector3d &coord) {
        return gridIndex2coord(coord2gridIndex(coord));
    }

    bool Astar3d::isOccupied(const Eigen::Vector3i &index) {
        Eigen::Vector3d pt3d; //
    ...
    inline void Astar3d::AstarGetSucc(GridNodePtr currentPtr, vector<GridNodePtr> &neighborPtrSets, vector<double> &edgeCostSets) {
    ...
    double Astar3d::getHeu(GridNodePtr node1, GridNodePtr node2) {
    ...
    Astar3d::PlanningStatus Astar3d::search(Vector3d start_pt, Vector3d end_pt) {
        ...
        */
        ...
        while (!openSet.empty()) {
            ...
            */
            ...
            if (currentPtr->index == goalIdx) {
                ...
                return Astar3d::PlanningStatus::SUCCESS;
            }
        ...
        if ((time_2 - time_1).toSec() > 0.1) {
            ...
            // ROS_WARN("Time consume in Astar3d path finding is %f", (time_2 - time_1).toSec() );
            cout << "Timeout > 0.1, consumed time: " << (time_2 - time_1).toSec() << endl;
        }
        ROS_ERROR_THROTTLE(1, "Astar3d failed");
...
//    delete GridNodeMap; ?
        ...
        return Astar3d::PlanningStatus::FAILED;
    }

/*
*
*
STEP 8:  Finish the Astar3d::retrievePath and Astar3d::getPath
please write your code below
*      
*/
    vector<Vector3d> Astar3d::getPath() {
    ...
    vector<Vector3d> Astar3d::getLocalPath(const vector<Vector3d> &path) {
        ...
        for (auto i = 1; i < path_normalized.size(); i++) {
            ...
            if (pos(0) < local_min(0) || pos(0) > local_max(0) ||
                ...
                pos(2) < local_min(2) || pos(2) > local_max(2)) {   // terminate search when first outside box point detected
                break;
            } else {
    ...
    vector<Vector3d> Astar3d::pathSimplify(const vector<Vector3d> &path) {
    ...
    vector<Vector3d> Astar3d::getVisitedNodes() {
